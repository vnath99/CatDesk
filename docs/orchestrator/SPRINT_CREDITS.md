...
| T-0023D-RC1 | COMPLETED_VERIFIED | unavailable | 120-190 | Correction-only pass enforced local Ollama-only policy, atomic active-run reservation, fail-closed workspace lock, JSONL torn-tail repair, durable RunStart approval, supported-only criteria, frozen tool exposure, and environment-based startup auth. |
| T-0023D-RC1.1 | COMPLETED_VERIFIED | unavailable | 45-80 | Correction-only pass fixed failed-run lock release versus NEEDS_SUPERVISOR retention, replaced criteria keyword matching with an exact normalized v1 allowlist, and required explicit startup-script AuthToken cleanup. |
